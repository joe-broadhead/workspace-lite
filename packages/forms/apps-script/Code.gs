const TOKEN_ENV_NAME = 'GOOGLE_WORKSPACE_FORMS_PROXY_TOKEN'
...
function doPost(e) {
  ...
  if (isRateLimited(100, FormsService.requestWeight(body.action, body.params || {}))) {
  ...
  try {
    return respond(FormsService.handle(body.action, body.params || {}))
  } catch(ex) {
    const correlationId = Utilities.getUuid()
    console.error('[forms-proxy] correlationId=%s action=%s error=%s', correlationId, body.action, ex && ex.message ? ex.message : String(ex))
