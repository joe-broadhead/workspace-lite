...
export function registerSlidesBatchTool(server: { tool: Function }) {
  server.tool(
    'slides_batch',
    'Execute multiple operations in a single round-trip. Pass an ordered array of {action, params} objects. Actions are the same names used by individual tools (presentationGet, slideAdd, slideDelete, slideDuplicate, slideMove, textBoxInsert, imageInsert, shapeInsert, tableInsert, slideElementsList, elementDelete, elementGetText, elementFormatText, slideNotes, textReplaceAll). Operations execute sequentially; errors are collected per-operation. Requires presentationId.',
    slidesBatchSchema,
