...
export function registerSlidesBatchTool(server: ToolServer) {
  server.tool(
    'slides_batch',
    'Execute multiple operations in a single round-trip against one existing presentation. Pass an ordered array of {action, params} objects. Actions are the same names used by individual tools (presentationGet, slideAdd, slideDelete, slideDuplicate, slideMove, textBoxInsert, imageInsert, shapeInsert, tableInsert, lineInsert, slideElementsList, elementDelete, elementGetText, elementFormatText, slideNotes, textReplaceAll, slideBackground). Operations execute sequentially; errors are collected per-operation with partial-success metadata. Requires presentationId.',
    slidesBatchSchema,
