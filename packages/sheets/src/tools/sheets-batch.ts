...
export function registerSheetsBatchTool(server: ToolServer) {
  server.tool(
    'sheets_batch',
    'Execute multiple operations in a single round-trip against one existing spreadsheet. Pass an ordered array of {action, params} objects. Actions are the same names used by individual tools (spreadsheetGet, sheetAdd, sheetDelete, sheetRename, sheetCopy, rangeRead, rangeWrite, rowsAppend, rangeClear, rangeGetFormulas, rangeGetNotes, valuesBatchGet, rangeFormat, rangeMerge, rangeUnmerge, columnWidth, freezeRows, rangeSort, formulaSet, chartCreate, noteSet, conditionalFormatGet, dataValidationSet, rowsInsert, rowsDelete). Operations execute sequentially; errors are collected and returned per-operation with partial-success metadata.',
    sheetsBatchSchema,
