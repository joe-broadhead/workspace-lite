...
export function registerSheetsBatchTool(server: { tool: Function }) {
  server.tool(
    'sheets_batch',
    'Execute multiple operations in a single round-trip. Pass an ordered array of {action, params} objects. Actions are the same names used by individual tools (spreadsheetCreate, spreadsheetGet, sheetAdd, sheetDelete, sheetRename, sheetCopy, rangeRead, rangeWrite, rowsAppend, rangeClear, rangeGetFormulas, rangeGetNotes, rangeFormat, rangeMerge, rangeUnmerge, columnWidth, freezeRows, rangeSort, formulaSet, chartCreate, noteSet). Operations execute sequentially; errors are collected and returned per-operation.',
    sheetsBatchSchema,
