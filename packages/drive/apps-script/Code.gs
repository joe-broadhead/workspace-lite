function doGet(e) {
  if (e && e.parameter && e.parameter.bootstrap === '1') {
    ...
    markBootstrapped()
    return respond(ok({ status: 'bootstrapped', token: token, note: 'Save this token as GOOGLE_WORKSPACE_DRIVE_PROXY_TOKEN. This endpoint will not return the token again.' }))
  }
