const TOKEN_ENV_NAME = 'GOOGLE_WORKSPACE_CALENDAR_PROXY_TOKEN'
...
function doPost(e) {
  ...
  if (isRateLimited(100, CalendarService.requestWeight(body.action, body.params || {}))) {
  ...
  try {
    ...
    console.error('[calendar-proxy] correlationId=%s action=%s error=%s', correlationId, body.action, ex && ex.message ? ex.message : String(ex))
