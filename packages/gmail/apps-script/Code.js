function doGet(e) {
  if (e && e.parameter && e.parameter.bootstrap === '1') {
    if (isBootstrapped()) return respond(err('FORBIDDEN', 'Bootstrap has already been completed.'))
    markBootstrapped()
    return respond(ok({ status: 'bootstrapped', token: getOrCreateToken(), note: 'Save as GOOGLE_WORKSPACE_GMAIL_PROXY_TOKEN. One-time use.' }))
  }
  return respond(ok({ status: 'healthy', version: '1.0.0', service: 'google-workspace-proxy-gmail' }))
}
...
function doPost(e) {
  ...
  try { return respond(GmailService.handle(body.action, body.params || {})) }
  catch(ex) {
    console.error('[gmail-proxy] action=%s error=%s', body.action, ex.message || String(ex))
