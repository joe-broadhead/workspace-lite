const TOKEN_ENV_NAME = 'GOOGLE_WORKSPACE_GMAIL_PROXY_TOKEN'
...
function doPost(e) {
  ...
  if (isRateLimited(100, GmailService.requestWeight(body.action, body.params || {}))) {
  ...
  try {
    return respond(GmailService.handle(body.action, body.params || {}))
  } catch(ex) {
    ...
    console.error('[gmail-proxy] correlationId=%s action=%s error=%s', correlationId, body.action, ex && ex.message ? ex.message : String(ex))
