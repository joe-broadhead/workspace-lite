...
| Property | Value |
|----------|-------|
| Token format | 48 random alphanumeric characters (A-Z, a-z, 0-9) |
| Token storage | `PropertiesService.getScriptProperties()` &mdash; persists across deployments |
| Token extracted from | JSON body (`body.token`), query param (`e.parameter.token`), or `Authorization: Bearer` header |
...
Each Apps Script proxy enforces **100 requests per minute** using `CacheService.getScriptCache()`.
...
| Response on limit | `{ success: false, error: { code: "RATE_LIMITED", message: "Try again in 60 seconds." } }` |
...
!!! tip "Handling rate limits"
    The MCP server will throw an error on rate-limited responses. The agent should retry after 60 seconds. For high-throughput workflows, use `{service}_batch` tools to pack up to 20 operations into a single request.
