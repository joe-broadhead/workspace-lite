...
| `min`/`max` on counts and sizes | Protects against oversized requests. Sheets operations also have proxy-enforced cell limits; Gmail body fields are capped at 100KB. |
...
│  3. getOrCreateToken()                  │
│     → Generate UUID-derived token       │
│     → Store in ScriptProperties         │
│  4. markBootstrapped() → set property   │
│  5. Return { token: "abc...xyz" }       │
└─────────────────────────────────────────┘
                      │
                      ▼
              Save as env var
...
| Property | Value |
|----------|-------|
| Token format | `Utilities.getUuid()` plus a second UUID with dashes removed |
...
```javascript
function isRateLimited(maxWeight, weight) {
  var cache = CacheService.getScriptCache()
  var key = 'rate_' + getCurrentAuthRateKey()
...
| Rate limit impact | Counts by action weight | Counts by the sum of child action weights |
