...
The primary token's default authorization classes in Auth.gs (`DEFAULT_AUTH_TOKEN_CLASSES`) are the full set: `read,draft,write,destructive,share,send`. This matches the deployed bootstrap default — docs must not claim a `read,draft`-only default. Operators who want a narrower primary token set `PROXY_AUTH_TOKEN_CLASSES` in Script Properties, or configure class-scoped tokens (`PROXY_WRITE_TOKEN`, `PROXY_SEND_TOKEN`, `PROXY_SHARE_TOKEN`, `PROXY_DESTRUCTIVE_TOKEN`, `PROXY_ADMIN_TOKEN`). MCP servers and the `wslite` CLI prefer matching environment variables such as `GOOGLE_WORKSPACE_DRIVE_PROXY_WRITE_TOKEN` or `GOOGLE_WORKSPACE_GMAIL_PROXY_SEND_TOKEN` for higher-risk actions, then fall back to `GOOGLE_WORKSPACE_<SERVICE>_PROXY_ADMIN_TOKEN`, then the primary `GOOGLE_WORKSPACE_<SERVICE>_PROXY_TOKEN`.
...
| Forms IDs | `^[a-zA-Z0-9_-]+$` | `<form-id>` / `<spreadsheet-id>` |
...
9. Proxy executes the action using Apps Script built-in or Advanced services (DriveApp, GmailApp, Tasks, FormApp, etc.)
